                    ...
                    switch (uci.command)
                    {
                        case "position":
                            ...
                            if (String.IsNullOrEmpty(lastFen))
                            {
                                ...
                                if (bookWrite && book.chess.Is2ToEnd(out string myMove, out string enMove))
                                {
                                    ...
                                    int del = Math.Max(movesDel, book.DeltaRecords());
                                    deleted += book.Delete(del);
                                    bookChanged = true;
                                }
                            }
                            break;
                        case "go":
                            ...
                            if ((plyRead == 0) || (plyRead > book.chess.halfMove))
                                move = book.GetMove(lastFen, lastMoves, bookRandom, ref bookWrite, ref lastMat);
                            if (string.IsNullOrEmpty(move))
                            {
                                if (bookLoaded && isW && String.IsNullOrEmpty(lastFen) && upAdd && (emptyRow == 0))
                                {
                                    int upCount = upMoves.Split(' ').Length;
                                    upCount += Math.Abs(upMat) * 2;
                                    if (upMat < 0)
                                        upCount++;
                                    upCount -= 2;
                                    added += book.AddUciMate(upMoves, upCount);
                                    bookChanged = true;
                                }
                                emptyRow++;
                                if (emptyRow > movesAdd)
                                    upAdd = false;
