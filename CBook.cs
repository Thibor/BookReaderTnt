    ...
    class CBook
    {
        ...
        public bool SaveToTnt(string p)
        {
            ...
            if (recList.Count / 100 != Program.bookCount / 100)
                log.Add($"book {recList.Count:N0} added {Program.added} deleted {Program.deleted:N0} zero {Zero()}");
            Program.bookCount = recList.Count;
            return true;
        }
        ...
        public int UpdateRec(CRec rec)
        {
            ...
            if (emoList.Count > 0)
            {
                CRec cr = emoList[0].rec;
                short mat = (short)-cr.mat;
                if (mat > 0)
                    mat++;
                if (rec.mat != mat)
                {
        ...
        public int AddUciMate(string moves, int countMoves = 0)
        {
            return AddUciMate(moves.Trim().Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries), countMoves);
        }

        public int AddUciMate(List<string> moves, int countMoves = 0)
        {
           return AddUciMate(moves.ToArray(), countMoves);
        }
        ...
        public string GetMove(string fen, string moves, int rnd, ref bool bookWrite, ref int mat)
        {
            chess.SetFen(fen);
            ...
            if (bst.rec != null)
            {
                mat = bst.rec.mat;
                if (mat != 0)
                    Console.WriteLine($"info score mate {mat}");
        ...
        public void Update()
        {
            ...
            Console.WriteLine($"records {recList.Count:N0} added {Program.added} deleted {Program.deleted:N0}");
        }
